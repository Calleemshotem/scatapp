...
const Sidebar = ({ activeTab, onTabChange, playlists, onPlaylistClick, isDarkTheme }) => {
  return (
    <aside className={`hidden md:flex flex-col w-64 ${isDarkTheme ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'} border-r h-screen fixed left-0 top-0 overflow-y-auto animate-slideInLeft transition-colors`}>
      {/* Logo */}
      ...
      <nav className="flex-1 p-4 space-y-2">
        <button
          ...
          className={`w-full text-left px-4 py-3 rounded transition transform hover:scale-105 ${
            activeTab === 'discover'
          ...
          <div className="space-y-1 mt-2">
            {playlists.map((playlist) => (
              <button
                ...
                className={`w-full text-left px-4 py-2 rounded text-sm transition transform hover:scale-105 ${
