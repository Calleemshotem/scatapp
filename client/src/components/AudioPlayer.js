...
const AudioPlayer = () => {
  ...
  return (
    <div className="relative">
      ...
      <button
        type="button"
        onPointerDown={() => setRepeat(!repeat)}
        onClick={() => setRepeat(!repeat)}
        className={`fixed bottom-28 right-4 z-50 rounded-full px-4 py-2 text-sm font-semibold transition cursor-pointer ${buttonClasses}`}
        title="Repeat"
      >
